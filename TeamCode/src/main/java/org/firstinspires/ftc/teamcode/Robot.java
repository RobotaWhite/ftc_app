...
*/
    ...
    public void autoDump(){
        blockDump.setPower(0.5);
        sleep(500);
        blockDump.setPower(0);
    }

    private void noChop(){
        servoColorCommand(1);
        sleep(300);
        servoColorCommand(0.8);
        sleep(300);
        servoColorCommand(0.6);
        sleep(300);
        servoColorCommand(0.4);
        sleep(300);
        servoColorCommand(0.2);
        sleep(700);
    }
    ...
    //toggles the wheels with a button input using the toggle class
    public void wheelIntake(boolean input) {
        if (intakeMotorToggle.value(input)) {
            //set intake motor speed here
            intakeLeft.setPower(.6);
            intakeRight.setPower(-1);
        } else {
            //turn off intake motors here
            intakeLeft.setPower(0);
            intakeRight.setPower(0);
        }
    }

    //moves the center v to put blocks in the boxes
    public void dump(boolean up, boolean down, double downPower, double upPower){
        if (down){
            blockDump.setPower(downPower);
        } else if (up){
            blockDump.setPower(-upPower);
        } else {
            blockDump.setPower(0);
        }
    }
    ...
    //puts the color servo down
    public void servoColorCommand(boolean up, boolean down) {
        if (up) {
            servoColorCommand(1);
        } else if (down) {
            servoColorCommand(0.2);
    ...
    //sets up the servo color command that actually puts it down
    private void servoColorCommand(double position) {
        servoColorPosition = position;
        servoColor.setPosition(servoColorPosition);
    }

    public void relicServoCommand(boolean in, boolean out){
        if (in){
	...
	//lifts the center out of the bot with the winches
	public void lift(boolean up, boolean down, double speedUp, double speedDown){
        if(up){
           motor_winch1.setPower(speedUp);
            motor_winch2.setPower(-speedUp);
        } else if (down){
            motor_winch1.setPower(-speedDown);
            motor_winch2.setPower(speedDown);
        } else {
            motor_winch1.setPower(0);
            motor_winch2.setPower(0);
        }
    }
    public void relicArm (boolean out, boolean in, double speed){
        if(out){
    ...
    public void TranslateReset(){
        nextState = 0;
        lastRightTranslate = false;
        lastLeftTranslate = false;
    }

    //these are variables for the translate function
    static final double RobotDriveCenterToCenter = 16; //This is a measuremet from left track center to right track center
    static final double OneturnCircumference = 2 * RobotDriveCenterToCenter * 3.14159;
    static final double DEGREES_PER_INCH = (1 / OneturnCircumference) * 360;
    static final double COUNTS_PER_DEGREE = COUNTS_PER_INCH / DEGREES_PER_INCH;
    private int currentState = -1;
    private int nextState = 0;

    //lets us operate our tracks kind of like mechanum wheels with this subroutine
    public void Translate(boolean left, boolean right, double power, double angle) {
        currentState = nextState;

        int pulses = (int) angle * (int) COUNTS_PER_DEGREE;
        if (left) {
            lastLeftTranslate = true;
            lastRightTranslate = false;
        } else if (right) {
            lastLeftTranslate = false;
            lastRightTranslate = true;
        }

        boolean moveDone = !(motorRearLeft.isBusy() && motorRearRight.isBusy());

        switch (currentState) {
            case 0:

                if (lastRightTranslate || lastLeftTranslate) {
                    nextState = 5;
                }
                break;
            ...
            case 5:
                ...
                if (lastRightTranslate) {
                    nextState = 10;
                } else if (lastLeftTranslate) {
                    nextState = 40;
                }
                break;
            case 10:
                ...
                if (lastRightTranslate) {
                    motorRearRight.setTargetPosition(0);
                    motorRearLeft.setTargetPosition(-pulses);
                    if (moveDone) {
                        nextState = 20;
                    }
                } else if (lastLeftTranslate) {
                    motorRearRight.setTargetPosition(0);
                    motorRearLeft.setTargetPosition(0);
                    if (moveDone) {
                        nextState = 40;
                        lastRightTranslate = false;
                        if (!left) {
            ...
            case 20:
                ...
                if (lastRightTranslate) {
                    motorRearRight.setTargetPosition(-pulses);
                    motorRearLeft.setTargetPosition(-pulses);
                    if (moveDone) {
                        nextState = 30;
                    }
